...
function auditOwnership(botIds, profiles) {
  const rows = (botIds || []).map((bot) => typeof bot === "string" ? { id: bot } : bot).filter(Boolean);
  const excluded = rows.filter((bot) => bot.migrationExcludedAt).map((bot) => bot.id);
  const activeIds = rows.filter((bot) => !bot.migrationExcludedAt).map((bot) => bot.id);
  ...
  const missing = activeIds.filter((botId) => !owners.has(botId));
  return { conflicts, missing, excluded, owned: [...owners].filter(([botId, values]) => activeIds.includes(botId) && new Set(values).size === 1).length };
}
...
async function run() {
  ...
  const result = { generatedAt: new Date().toISOString(), ...auditOwnership(registry.getAllClients().map((client) => ({ id: client.id, migrationExcludedAt: client.migrationExcludedAt })), profiles) };
