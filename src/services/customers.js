...
function profilePath(clientId, userId) {
  return path.join(DATA_DIR, safeId(clientId), "customers", `${safeId(userId)}.json`);
}
...
function getProfile(clientId, userId) {
  const file = profilePath(clientId, userId);
  if (!fs.existsSync(file)) return emptyProfile(userId);
  return JSON.parse(fs.readFileSync(file, "utf8"));
}

function saveProfile(clientId, profile) {
  const file = profilePath(clientId, profile.userId);
